...
if run_btn:
    st.session_state.log = []
    result_df = df.copy()
    empty_rows = df[df[target_col] == ""].index.tolist()
    ...
    for b in range(0, total, BATCH_SIZE):
        chunk_idx = empty_rows[b: b + BATCH_SIZE]
        rows_payload = []
        for ri in chunk_idx:
            row_dict = {col: str(df.at[ri, col]) for col in df.columns}
            rows_payload.append({"rowNumber": int(ri) + 2, "data": row_dict})

        system_prompt = (
            f'You are an expert data analyst filling missing Excel values.\n'
            f'Target column: "{target_col}"\n'
            f'User instruction: {instruction}\n'
            f'All column headers: {", ".join(df.columns.tolist())}\n'
            f'Rules:\n'
            f'- Return ONLY a valid JSON array, no explanation, no markdown\n'
            f'- Match the format/style of existing values in the column\n'
            f'- If no prior values exist, follow the instruction\n'
            f'- Format: [{{"rowNumber": N, "value": "..."}}]'
        )
        user_msg = f'Fill "{target_col}" for these rows:\n{json.dumps(rows_payload, ensure_ascii=False, indent=2)}'

        try:
            logs.append({"m": f"Processing rows {b+2}–{min(b+BATCH_SIZE, total)+1}…", "t": "info"})
            render_log()
            ...
            if parsed and isinstance(parsed, list):
                for item in parsed:
                    rn = item.get("rowNumber", -1) - 2
                    val = str(item.get("value", ""))
                    if 0 <= rn < len(result_df):
                        result_df.at[rn, target_col] = val
                        logs.append({"m": f"Row {rn+2}: {val[:80]}", "t": "success"})
            else:
                logs.append({"m": f"Could not parse response for batch {b//BATCH_SIZE+1}", "t": "error"})

        except Exception as e:
            logs.append({"m": f"Error: {str(e)}", "t": "error"})

        done += len(chunk_idx)
        progress_bar.progress(done / total, text=f"{done}/{total} cells filled")
        render_log()
