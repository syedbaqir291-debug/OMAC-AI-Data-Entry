...
st.markdown("**📎 Reference Columns** — AI will use these as context when filling")
other_cols = [c for c in df.columns if c != target_col]

# Smart defaults: top 6 most-filled columns
default_fill_rate = {c: (df[c].replace("", None).notna().sum() / max(len(df), 1)) for c in other_cols}
smart_defaults = [c for c, r in sorted(default_fill_rate.items(), key=lambda x: -x[1]) if r > 0.1][:6]

ref_cols = st.multiselect(
    "Select one or more reference columns (AI reads these to generate values)",
    options=other_cols,
    default=smart_defaults,
    placeholder="Choose reference columns…",
)

if not ref_cols:
    st.markdown('<div class="warn-card">⚠ No reference columns selected — AI will rely only on your instruction text.</div>', unsafe_allow_html=True)
else:
    st.caption(f"✓ {len(ref_cols)} column(s) selected: {', '.join(ref_cols[:6])}{'…' if len(ref_cols) > 6 else ''}")
...
if run_btn:
    ...
    # ── Use user-selected reference columns; fall back to auto-pick if none chosen ──
    if ref_cols:
        useful_cols = [c for c in ref_cols if c in work_df.columns]
        logs.append({"m": f"Using {len(useful_cols)} user-selected reference column(s): {', '.join(useful_cols[:5])}{'…' if len(useful_cols)>5 else ''}", "t": "info"})
    else:
        col_fill_rate = {c: (work_df[c].apply(lambda v: not is_blank(v)).sum() / max(len(work_df), 1))
                         for c in work_df.columns if c != target_col}
        useful_cols = [c for c, r in sorted(col_fill_rate.items(), key=lambda x: -x[1]) if r > 0.1][:10]
        if not useful_cols:
            useful_cols = list(work_df.columns[:10])
        logs.append({"m": f"No reference columns selected — auto-picked: {', '.join(useful_cols[:5])}", "t": "info"})

    def trim(v, n=300):
        s = str(v).strip()
        return s[:n] + "…" if len(s) > n else s

    SAFE_BATCH = 3  # smaller batches = smaller payloads = no 400 errors

    for b in range(0, total, SAFE_BATCH):
        chunk_idx = empty_rows[b : b + SAFE_BATCH]
        rows_payload = []
        for ri in chunk_idx:
            row_dict = {col: trim(work_df.at[ri, col]) for col in useful_cols}
            rows_payload.append({"_rowIdx": ri, "data": row_dict})
        ...
        system_prompt = (
            ...
            f'- Keep "_rowIdx" exactly as given\n'
            f'- Format: [{{"_rowIdx": N, "value": "..."}}]'
        )
        user_msg = (
            f'Fill "{target_col}" for these {len(chunk_idx)} rows. '
            f'Return exactly {len(chunk_idx)} JSON items.\n'
            + json.dumps(rows_payload, ensure_ascii=False)
        )

        try:
            logs.append({"m": f"Batch {b//SAFE_BATCH+1}: rows {chunk_idx[0]+2}–{chunk_idx[-1]+2}  ({len(chunk_idx)} cells)…", "t": "info"})
            render_log()
            ...
            if parsed and isinstance(parsed, list):
                ...
                logs.append({"m": f"Batch {b//SAFE_BATCH+1} parse failed — retrying row by row…", "t": "error"})
                for ri in chunk_idx:
                    row_dict = {col: trim(work_df.at[ri, col]) for col in useful_cols}
                    retry_msg = (
                        f'Fill the single missing value for column "{target_col}".\n'
